...
def multiresol_patchify2D(edge, image, patch_size, is_multiresol=True):
    ...
    if is_multiresol and torch.rand(1).item() < 0.2:
        possible_h = torch.arange(patch_size, h, patch_size)
        possible_w = torch.arange(patch_size, w, patch_size)
        th = possible_h[torch.randint(0, len(possible_h), (1,))].item()
        tw = possible_w[torch.randint(0, len(possible_w), (1,))].item()
        ni, nj = th // patch_size, tw // patch_size
    else:
    ...
    if is_multiresol:
        edge_patch = edge_patch[:, ::ni, ::nj]
        image_patch = image_patch[:, ::ni, ::nj]
        x_pos = x_pos[::ni]
        y_pos = y_pos[::nj]
...
def multiresol_patchify3D(edge, image, patch_size, is_multiresol=True):
    ...
    if is_multiresol and torch.rand(1).item() < 0.2:
        possible_h = torch.arange(patch_size, h, patch_size)
        possible_w = torch.arange(patch_size, w, patch_size)
        possible_d = torch.arange(patch_size, d, patch_size)
        th = possible_h[torch.randint(0, len(possible_h), (1,))].item()
        tw = possible_w[torch.randint(0, len(possible_w), (1,))].item()
        td = possible_d[torch.randint(0, len(possible_d), (1,))].item()
        ni, nj, nk = th // patch_size, tw // patch_size, td // patch_size
    else:
    ...
    if is_multiresol:
        edge_patch = edge_patch[:, ::ni, ::nj, ::nk]
        image_patch = image_patch[:, ::ni, ::nj, ::nk]
        x_pos = x_pos[::ni]
        y_pos = y_pos[::nj]
        z_pos = z_pos[::nk]
...
def multiresol_patchify3D_(edge, image, patch_size, is_multiresol=True):
    assert edge.shape == image.shape
